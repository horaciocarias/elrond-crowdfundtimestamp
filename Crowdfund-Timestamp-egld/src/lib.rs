...
//contract expires at midnight 8-May-2022
const CONTRACT_DEADLINE: u64 = 1651982400;
...
pub trait Crowdfunding {
	...
	fn status(&self) -> Status {
		if self.get_block_timestamp()  <= self.get_deadline() {
    ...
    fn init(&self, target: &BigUint) {
		...
		let deadline = CONTRACT_DEADLINE;
